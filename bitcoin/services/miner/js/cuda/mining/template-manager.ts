...
import {
  createEnhancedRPCConfig,
  type EnhancedRPCConfig,
  getBlockTemplate as getEnhancedBlockTemplate,
} from "../rpc/enhanced-client.ts";
...
interface TemplateManagerState {
  enhancedRPCConfig: EnhancedRPCConfig;
  isRunning: boolean;
...
export function createTemplateManager(
  ...
  async function fetchTemplate(): Promise<Result<BlockTemplate>> {
    if (config.mode.type === "genesis") {
      ...
      // Fetch live template from Bitcoin Core using enhanced client with retry logic
      const enhancedConfig = createEnhancedRPCConfig(config.rpcConfig);
      return await getEnhancedBlockTemplate(enhancedConfig);
    }
  }
