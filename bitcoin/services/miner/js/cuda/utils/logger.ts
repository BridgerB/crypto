...
export function logMiningStatus(stats: MiningStats, logger: Logger): void {
  ...
  // Calculate extended mining metrics
  const targetSearchSpace = 281474976710656; // 2^48 attempts
  const searchSpacePercent = Math.min(
    (stats.totalHashes / targetSearchSpace) * 100,
    100,
  );
  const effectiveHashRate = stats.totalHashRate > 0 ? stats.totalHashRate : 0;

  logger.info(
    `${LOGGING_CONSTANTS.EMOJIS.LIGHTNING} Mining Status: ${stats.totalHashes.toLocaleString()} attempts | ` +
      `${effectiveHashRate.toLocaleString()} H/s | ` +
      `${stats.workersActive} workers active | ` +
      `${searchSpacePercent.toFixed(3)}% of 2^48 search space | ` +
      `${elapsedSeconds}s elapsed`,
  );
}

export function logExtendedMiningProgress(
  totalWorkers: number,
  ...
  const targetSearchSpace = 281474976710656; // 2^48 attempts
  const searchSpacePercent = Math.min(
    (totalAttempts / targetSearchSpace) * 100,
    100,
  );

  // Estimate time to completion based on current hash rate
  const remainingAttempts = Math.max(0, targetSearchSpace - totalAttempts);
  const estimatedSecondsRemaining = totalHashRate > 0
    ? remainingAttempts / totalHashRate
    : Infinity;
  const estimatedHoursRemaining = estimatedSecondsRemaining / 3600;

  // Calculate block finding probability (corrected for Bitcoin mainnet)
  // Bitcoin difficulty of 136T means we need ~136T * 2^32 attempts on average
  const difficultyTarget = difficulty * Math.pow(2, 32); // Correct conversion
  const blockFindingPercent = (totalAttempts / difficultyTarget) * 100;

  logger.info(
    `📊 Extended Mining Progress: ` +
      `${totalAttempts.toLocaleString()} total attempts | ` +
      `${totalHashRate.toLocaleString()} combined H/s | ` +
      `${searchSpacePercent.toFixed(4)}% of 2^48 space`,
  );

  if (estimatedHoursRemaining < Infinity && estimatedHoursRemaining > 0) {
    logger.info(
      `⏱️  Estimated time to complete 2^48 space: ${
        estimatedHoursRemaining.toFixed(1)
      } hours | ` +
        `Block finding probability: ${blockFindingPercent.toFixed(6)}%`,
    );
  }
}
